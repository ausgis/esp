...
The error term is specified as $\mu \sim \mathcal{N}(0, 0.1)$, and the corresponding $Y$ is simulated using the spatial durbin process. The autoregressive coefficients of X are set to `0.5`. The spatial lag coefficient takes values from 0.05 to 0.95 with an interval of 0.05. For each spatial lag coefficient $\rho$, we randomly simulate 1000 times.
...
lw = spdep::nb2listw(sdsfun::spdep_nb(simx),
                     style = "W",
                     zero.policy = TRUE)
wxg = spdgp::make_wxg(spdgp::make_wx(as.data.frame(simx$simx_strata), lw), 0.5)
```
...
##     <dbl>  <dbl>    <dbl> <dbl>
##  1  0.758  0.936 -0.178    0.05
##  2  0.853  0.935 -0.0823   0.05
##  3  0.936  0.933  0.00246  0.05
##  4  0.942  0.936  0.00614  0.05
##  5  0.881  0.927 -0.0454   0.05
##  6  0.878  0.919 -0.0417   0.05
##  7  0.935  0.940 -0.00432  0.05
##  8  0.934  0.932  0.00252  0.05
##  9  0.934  0.941 -0.00666  0.05
## 10  0.928  0.933 -0.00475  0.05
## # ℹ 18,990 more rows
tapply(qv$qv_bias, qv$rho, mean)
##        0.05         0.1        0.15         0.2        0.25         0.3        0.35         0.4 
## -0.05682187 -0.06284893 -0.06126073 -0.06811650 -0.07483771 -0.08105812 -0.09004364 -0.09702247 
##        0.45         0.5        0.55         0.6        0.65         0.7        0.75         0.8 
## -0.10507023 -0.11487207 -0.11847570 -0.12709023 -0.13370683 -0.14249375 -0.15056883 -0.16725110 
##        0.85         0.9        0.95 
## -0.21084108 -0.23360922 -0.24126673
```
