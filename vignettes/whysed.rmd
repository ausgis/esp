...
mc_simq = \(cores = 6){
  ...
  calcul_q = \(paramgd){
    ...
    qv = sesp::sesp(y ~ ., data = sf::st_set_geometry(dt,sf::st_geometry(simx)), listw = lw,
...
qv = mc_simq(cores = 12)
## Error in checkForRemoteErrors(val): 12 nodes produced errors; first error: The listw needs to be preserved in a list.
```
