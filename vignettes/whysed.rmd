...
```{r real_stratification_x,fig.width=3.75,fig.height=3,fig.cap=knitr::asis_output("**Figure 1**. Strata of simulated independent variable X")}
...
ggplot2::ggplot() +
  ggplot2::geom_sf(data = simx,
                   ggplot2::aes(color = factor(simx_zone))) +
...
zone = sf::st_sf(zones = c(1,2,3),
                 geometry = do.call(c, list(zone1,zone2,zone3)))
```
...
## A Monte Carlo experiment to demonstrate the advantages of spatially explicit discretization.
