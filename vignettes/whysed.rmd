...
### The spatial autocorrelation of the simulated X strata
...
------------- ------------- ----------- ---------- ------- -----------
 simx_strata   0.308731***   -0.004464   0.001493   8.105   2.634e-16 
----------------------------------------------------------------------
...
The error term is specified as $\mu \sim \mathcal{N}(0, 0.1)$, and the corresponding $Y$ is simulated using the spatial durbin process. The autoregressive coefficients of X are set to `1`. The spatial lag coefficient takes values from 0.05 to 0.95 with an interval of 0.05. For each spatial lag coefficient $\rho$, we randomly simulate 1000 times.
...
lw = spdep::nb2listw(sdsfun::spdep_nb(simx),
                     style = "W",
                     zero.policy = TRUE)
wxg = spdgp::make_wxg(spdgp::make_wx(as.data.frame(simx$simx_strata), lw), 1)
```
...
mc_simq = \(cores = 6){
  ...
  calcul_q = \(paramgd) {
    rhov = paramgd[[1]]
    u_i = paramgd[[2]]
    y = spdgp::sim_durbin(us[[u_i]], simx$simx_strata, wxg, lw, rho = rhov)
    sed = hclustgeo_disc(y,alpha = 0.05)
  ...
  if (doclust) {
    parallel::clusterExport(cores, varlist = c("simx","us","rho","lw","wxg"))
...
##     <dbl>  <dbl>    <dbl> <dbl>
##  1  0.904  0.886  0.0180   0.05
##  2  0.882  0.901 -0.0188   0.05
##  3  0.866  0.865  0.00129  0.05
##  4  0.852  0.909 -0.0571   0.05
##  5  0.907  0.905  0.00125  0.05
##  6  0.897  0.903 -0.00581  0.05
##  7  0.890  0.911 -0.0210   0.05
##  8  0.881  0.884 -0.00306  0.05
##  9  0.903  0.906 -0.00244  0.05
## 10  0.900  0.894  0.00611  0.05
## # ℹ 18,990 more rows
tapply(qv$qv_bias, qv$rho, mean)
##          0.05           0.1          0.15           0.2          0.25           0.3          0.35 
## -0.0084092253 -0.0099579078 -0.0090435990 -0.0087835037 -0.0065889193 -0.0058471828 -0.0057258774 
##           0.4          0.45           0.5          0.55           0.6          0.65           0.7 
## -0.0060168128 -0.0036609651 -0.0049905972 -0.0050491521 -0.0069466575 -0.0013470179 -0.0012837499 
##          0.75           0.8          0.85           0.9          0.95 
## -0.0027753109 -0.0035111783 -0.0041575292 -0.0008531478 -0.0043249266
```
