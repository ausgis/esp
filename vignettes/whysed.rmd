...
The q-value obtained by classifying into five categories using the natural breaks method is `r round(qv_naturaldisc, 5)`, while the q-value from five-category hierarchical clustering without spatial soft constraints is `r round(qv_hclustdisc, 5)`. In the Monte Carlo simulation experiment, the range of q-values spans from `r round(min(qv$qv_sed), 5)` to `r round(max(qv$qv_sed), 5)`. The range of q-values from randomized simulations that best align with actual conditions is from `r round(min(qv_sign$qv_sed), 5)` to `r round(max(qv_sign$qv_sed), 5)`. The Monte Carlo simulation results suggest that spatially implicit discretization tends to underestimate the q-value of the variable when moderate spatial autocorrelation is present, emphasizing the need for spatially explicit discretization.

<!-- In the repeated experiment, X is shuffled 10000 times, and the global Moran's I for the shuffled X is calculated. If the Moran's I of the shuffled X is significant and greater than 0, it is used as the degree of spatial soft constraint for the spatially constrained hierarchical clustering. Otherwise, the degree of soft constraint is set to 0. Subsequently, the q-values for each simulation round is computed. -->

<!-- ```{r mc_simq1} -->
<!-- mc_simq1 = \(times = 5000, cores = 6){ -->
<!--   doclust = FALSE -->
<!--   if (inherits(cores, "cluster")) { -->
<!--     doclust = TRUE -->
<!--   } else if (cores > 1) { -->
...
<!-- qv1 = mc_simq1(times = 10000, cores = 12) |> -->
