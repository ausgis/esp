...
The error term is specified as $\mu \sim \mathcal{N}(0, 0.1)$, and the corresponding $Y$ is simulated using the spatial durbin process. The autoregressive coefficients of X are set to `0.35`. The spatial lag coefficient takes values from 0.05 to 0.95 with an interval of 0.05. For each spatial lag coefficient $\rho$, we randomly simulate 1000 times.
...
lw = spdep::nb2listw(sdsfun::spdep_nb(simx),
                     style = "W",
                     zero.policy = TRUE)
wxg = spdgp::make_wxg(spdgp::make_wx(as.data.frame(simx$simx_strata), lw), 0.35)
```
...
##     <dbl>  <dbl>     <dbl> <dbl>
##  1  0.953  0.957 -0.00399   0.05
##  2  0.775  0.961 -0.186     0.05
##  3  0.951  0.951 -0.000152  0.05
##  4  0.827  0.956 -0.129     0.05
##  5  0.955  0.954  0.000850  0.05
##  6  0.890  0.957 -0.0674    0.05
##  7  0.958  0.956  0.00153   0.05
##  8  0.887  0.956 -0.0687    0.05
##  9  0.887  0.958 -0.0706    0.05
## 10  0.898  0.956 -0.0580    0.05
## # ℹ 18,990 more rows
tapply(qv$qv_bias, qv$rho, mean)
##        0.05         0.1        0.15         0.2        0.25         0.3        0.35         0.4 
## -0.04942512 -0.04973972 -0.05588335 -0.05714212 -0.05829034 -0.06436342 -0.07409624 -0.08317897 
##        0.45         0.5        0.55         0.6        0.65         0.7        0.75         0.8 
## -0.08968615 -0.09808929 -0.10761798 -0.11693319 -0.12293627 -0.13167992 -0.14371166 -0.16074675 
##        0.85         0.9        0.95 
## -0.19978206 -0.23137667 -0.24043913
```
