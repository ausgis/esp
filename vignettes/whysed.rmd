...
The error term is specified as $\mu \sim \mathcal{N}(0, 0.1)$, and the corresponding $Y$ is simulated using the spatial durbin process. The autoregressive coefficients of X are set to match the stratification levels, i.e., 1, 2, and 3. The spatial lag coefficient takes values from 0.05 to 0.95 with an interval of 0.05. For each spatial lag coefficient $\rho$, we randomly simulate 100 times.
...
mc_simq = \(cores = 6){
  ...
  hclustgeo_disc = \(xv){
    D0 = stats::dist(xv)
    resh = ClustGeo::hclustgeo(D0,gdist,alpha = 0.5)
  ...
  calcul_q = \(paramgd){
    rhov = paramgd[[1]]
    u_i = paramgd[[2]]
    y = spdgp::sim_durbin(us[[u_i]], simx$simx_strata, simx$simx_strata, lw, rho = rhov)
    sed = hclustgeo_disc(y)
    sid = sdsfun::discretize_vector(y,3,method = 'natural')
...
##     <dbl>  <dbl>   <dbl> <dbl>
##  1  0.995  0.984  0.0110  0.05
##  2  0.995  0.984  0.0109  0.05
##  3  0.995  0.984  0.0113  0.05
##  4  0.994  0.983  0.0112  0.05
##  5  0.995  0.984  0.0105  0.05
##  6  0.996  0.984  0.0119  0.05
##  7  0.996  0.985  0.0112  0.05
##  8  0.995  0.983  0.0118  0.05
##  9  0.996  0.986  0.0100  0.05
## 10  0.995  0.984  0.0107  0.05
## # ℹ 1,890 more rows
```
