...
The error term is specified as $\mu \sim \mathcal{N}(0, 0.1)$, and the corresponding $Y$ is simulated using the spatial lag process. The spatial lag coefficient takes values from 0.05 to 0.95 with an interval of 0.05. For each spatial lag coefficient $\rho$, we randomly simulate 100 times. 
...
mc_simq = \(cores = 6){
  ...
  gdist = stats::as.dist(sdsfun::sf_distance_matrix(data))
  hclustgeo_disc = \(xv){
    D0 = stats::dist(xv)
    resh = ClustGeo::hclustgeo(D0,gdist,alpha = 0.75)
  ...
  calcul_q = \(paramgd){
    rhov = paramgd[[1]]
    u_i = paramgd[[2]]
    y = spdgp::sim_sar(us[[u_i]], simx$simx_strata, lw, rho = rhov)
    dt = tibble::tibble(y = y, sed = hclustgeo_disc(y), 
                        sid = sdsfun::discretize_vector(y,3,method = 'natural'))
    qv = gdverse::gd(y~.,data = dt)
