...
----------- ------------- ----------- ---------- ------- -----------
 simx_zone   0.308731***   -0.004464   0.001493   8.105   2.634e-16 

   simxv     0.159677***   -0.004464   0.001493   4.248   1.079e-05 
--------------------------------------------------------------------
...
``` r
simx = dplyr::mutate(simx,wx = simx_zone * 0.1)
...
mc_simq = \(cores = 6){
  ...
  calcul_q = \(paramgd){
    ...
    qv = sesp::sesp(y ~ ., data = sf::sf::st_set_geometry(dt,sf::st_geometry(simx)),
                    listw = lw, discvar = "xv", discnum = 3,
...
qv = mc_simq(cores = 12)
## Error in parse(text = input): <text>:25:41: unexpected '::'
## 24:     dt$robustdisc = dplyr::pull(gdverse::robust_disc(y~xv,data = dt,discnum = 3),1)
## 25:     qv = sesp::sesp(y ~ ., data = sf::sf::
##                                             ^
```
